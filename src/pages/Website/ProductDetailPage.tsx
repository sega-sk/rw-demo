...
import { dummyProducts, dummyMemorabilia, dummyMerchandise } from '../../data/dummyData';
...
export default function ProductDetailPage() {
  ...
  const currentProduct = product || dummyProducts.find(p => p.slug === slug);
  const relatedProducts = relatedProductsData?.rows?.filter(p => p.id !== currentProduct?.id) || 
    dummyProducts.filter(p => p.id !== currentProduct?.id).slice(0, 4);
  
  // Use API data if available, otherwise use dummy data
  const memorabiliaItems = memorabiliaData?.rows?.length > 0 ? memorabiliaData.rows : dummyMemorabilia.slice(0, 6);
  const merchandiseItems = merchandiseData?.rows?.length > 0 ? merchandiseData.rows : dummyMerchandise.slice(0, 6);
