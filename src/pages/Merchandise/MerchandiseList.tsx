...
import { dummyMerchandise } from '../../data/dummyData';
...
export default function MerchandiseList() {
  ...
  useEffect(() => {
    const timeoutId = setTimeout(() => {
      if (searchTerm.trim() || merchandiseData?.rows?.length === 0) {
        refetchMerchandise();
      }
    }, 300);
  ...
  const merchandiseItems = merchandiseData?.rows?.length > 0 ? merchandiseData.rows : dummyMerchandise;
