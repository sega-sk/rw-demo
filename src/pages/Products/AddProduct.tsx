...
import { dummyMemorabilia, dummyMerchandise } from '../../data/dummyData';
...
export default function AddProduct() {
  ...
  // Add debug logging
  console.log('AddProduct component rendered', location.pathname, 'isAuthenticated:', isAuthenticated);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    console.log('Submitting product form with data:', JSON.stringify(formData, null, 2));
    ...
    try {
      ...
      const cleanedData = {
        ...
        // Convert string prices to numbers where needed
        sale_price: formData.sale_price ? parseFloat(formData.sale_price.toString()) : null,
        retail_price: formData.retail_price ? parseFloat(formData.retail_price.toString()) : null,
        rental_price_hourly: formData.rental_price_hourly ? parseFloat(formData.rental_price_hourly.toString()) : null,
        rental_price_daily: formData.rental_price_daily ? parseFloat(formData.rental_price_daily.toString()) : null,
        rental_price_weekly: formData.rental_price_weekly ? parseFloat(formData.rental_price_weekly.toString()) : null,
        rental_price_monthly: formData.rental_price_monthly ? parseFloat(formData.rental_price_monthly.toString()) : null,
        rental_price_yearly: formData.rental_price_yearly ? parseFloat(formData.rental_price_yearly.toString()) : null,
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="max-w-7xl mx-auto product-form-container">
        <div className="mb-6">
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-3 gap-6" id="product-form">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-lg shadow p-6">
              ...
              {(memorabiliaData?.rows?.length > 0 ? memorabiliaData.rows : dummyMemorabilia).slice(0, 3).map((item) => (
            ...
            <div className="bg-white rounded-lg shadow p-6">
              ...
              {/* Use API data if available, otherwise use dummy data */}
              {(merchandiseData?.rows?.length > 0 ? merchandiseData.rows : dummyMerchandise).slice(0, 3).map((item) => (
                <div key={item.id} className="flex items-center space-x-3 mb-3">
                  <input
                    type="checkbox"
        ...
        <div className="mt-6 flex justify-end space-x-4">
          ...
          <Button onClick={handleSubmit} loading={creating} className="btn-hover">
            Save Product
          </Button>
        </div>
      </div>
